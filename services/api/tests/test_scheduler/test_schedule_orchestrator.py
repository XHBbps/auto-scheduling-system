...
async def test_full_schedule_single_order(db_session):
    ...
    db_session.add(BomRelationSrc(
        ...
        bom_component_desc="机身MC1-80", bom_level=2,
...
async def test_missing_bom_blocks_schedule_and_records_issue(db_session):
    order = SalesPlanOrderLineSrc(
        sap_code="SAP999", sap_line_no="10",
        contract_no="HT999", customer_name="客户B",
        product_model="MC2-45", product_series="MC2",
        material_no="MACH999", quantity=Decimal("1"),
        order_no="SO999",
        confirmed_delivery_date=datetime(2026, 6, 30),
        drawing_released=True,
    )
    db_session.add(order)
    db_session.add(MachineCycleBaseline(
        machine_model="MC2-45", product_series="MC2",
        order_qty=Decimal("1"), cycle_days_median=Decimal("45"),
        sample_count=5, is_active=True,
    ))
    await db_session.commit()
...
async def test_missing_machine_cycle_records_issue_and_marks_abnormal(db_session):
    order = SalesPlanOrderLineSrc(
        sap_code="SAP888", sap_line_no="10",
        contract_no="HT888", customer_name="客户C",
        product_model="PSP-80B(二级传动)", product_series="PSP",
        material_no="MACH888", quantity=Decimal("1"),
        order_no="SO888",
        confirmed_delivery_date=datetime(2026, 6, 30),
        drawing_released=True,
    )
    db_session.add(order)
    db_session.add(BomRelationSrc(
        machine_material_no="MACH888", plant="1000",
        material_no="MACH888", bom_component_no="ASM_X",
    ...
    db_session.add(AssemblyTimeBaseline(
        machine_model="PSP-80B(二级传动)", assembly_name="机身",
        assembly_time_days=Decimal("2"), is_final_assembly=False,
        production_sequence=1,
    ))
    await db_session.commit()
...
async def test_pending_drawing_records_issue(db_session):
    order = SalesPlanOrderLineSrc(
        sap_code="SAP777", sap_line_no="10",
        contract_no="HT777", customer_name="客户D",
        product_model="MC1-80", product_series="MC1",
...
async def test_pending_delivery_records_issue(db_session):
    order = SalesPlanOrderLineSrc(
        sap_code="SAP778", sap_line_no="10",
        contract_no="HT778", customer_name="客户E",
        product_model="MC1-80", product_series="MC1",
        material_no="MACH778", quantity=Decimal("1"),
        order_no="SO778",
        confirmed_delivery_date=None,
        drawing_released=True,
    )
...
async def test_missing_part_cycle_records_issue_and_marks_abnormal(db_session):
    order = SalesPlanOrderLineSrc(
        sap_code="SAP666", sap_line_no="10",
        contract_no="HT666", customer_name="客户E",
        product_model="MC1-80", product_series="MC1",
        material_no="MACH666", quantity=Decimal("1"),
        order_no="SO666",
        confirmed_delivery_date=datetime(2026, 6, 30),
        drawing_released=True,
    )
    ...
    db_session.add(BomRelationSrc(
        machine_material_no="MACH666", plant="1000",
        material_no="MACH666", bom_component_no="ASM666",
        bom_component_desc="机身MC1-80", bom_level=1,
        is_top_level=True, is_self_made=True, part_type="自产件",
    ))
    db_session.add(BomRelationSrc(
        machine_material_no="MACH666", plant="1000",
        material_no="ASM666", bom_component_no="PART666",
    ...
    db_session.add(AssemblyTimeBaseline(
        machine_model="MC1-80", assembly_name="整机总装",
        assembly_time_days=Decimal("3"), production_sequence=99,
...
async def test_missing_assembly_time_records_issue_and_marks_abnormal(db_session):
    order = SalesPlanOrderLineSrc(
        sap_code="SAP555", sap_line_no="10",
        contract_no="HT555", customer_name="客户F",
        product_model="MC1-80", product_series="MC1",
        material_no="MACH555", quantity=Decimal("1"),
        order_no="SO555",
        confirmed_delivery_date=datetime(2026, 6, 30),
        drawing_released=True,
    )
    ...
    db_session.add(BomRelationSrc(
        machine_material_no="MACH555", plant="1000",
        material_no="MACH555", bom_component_no="ASM555",
        bom_component_desc="机身MC1-80", bom_level=1,
        is_top_level=True, is_self_made=True, part_type="自产件",
    ))
    db_session.add(BomRelationSrc(
        machine_material_no="MACH555", plant="1000",
        material_no="ASM555", bom_component_no="PART555",
        bom_component_desc="机身焊接件", bom_level=2,
        is_self_made=True, part_type="自产件",
    ))
    db_session.add(PartCycleBaseline(
        material_no="PART555", material_desc="机身焊接件",
        core_part_name="焊接件", machine_model="MC1-80",
...
async def test_batch_schedule_rolls_back_partial_data_when_single_order_raises(db_session, monkeypatch):
    order_ok = SalesPlanOrderLineSrc(
        sap_code="SAPB001", sap_line_no="10",
        contract_no="HTB001", customer_name="客户G",
        product_model="MC1-80", product_series="MC1",
        material_no="MACHB001", quantity=Decimal("1"),
        order_no="SOB001",
        confirmed_delivery_date=datetime(2026, 6, 30),
        drawing_released=True,
    )
    order_fail = SalesPlanOrderLineSrc(
        sap_code="SAPB002", sap_line_no="10",
        contract_no="HTB002", customer_name="客户H",
        product_model="MC1-80", product_series="MC1",
        material_no="MACHB002", quantity=Decimal("1"),
        order_no="SOB002",
        confirmed_delivery_date=datetime(2026, 6, 30),
        drawing_released=True,
    )
    ...
    db_session.add_all([
        BomRelationSrc(
            machine_material_no="MACHB001", plant="1000",
            material_no="MACHB001", bom_component_no="ASM_B001",
            bom_component_desc="机身MC1-80", bom_level=1,
            is_top_level=True, is_self_made=True, part_type="自产件",
        ),
        BomRelationSrc(
            machine_material_no="MACHB001", plant="1000",
            material_no="ASM_B001", bom_component_no="PART_B001",
            bom_component_desc="机身焊接件", bom_level=2,
            is_self_made=True, part_type="自产件",
        ),
        BomRelationSrc(
            machine_material_no="MACHB002", plant="1000",
            material_no="MACHB002", bom_component_no="ASM_B002",
            bom_component_desc="机身MC1-80", bom_level=1,
            is_top_level=True, is_self_made=True, part_type="自产件",
        ),
        BomRelationSrc(
            machine_material_no="MACHB002", plant="1000",
            material_no="ASM_B002", bom_component_no="PART_B002",
            bom_component_desc="机身焊接件", bom_level=2,
            is_self_made=True, part_type="自产件",
        ),
    ])
    ...
    db_session.add_all([
        PartCycleBaseline(
            material_no="PART_B001", material_desc="机身焊接件",
            core_part_name="焊接件", machine_model="MC1-80",
            ref_batch_qty=Decimal("1"), cycle_days=Decimal("8"),
            unit_cycle_days=Decimal("8"), is_active=True,
        ),
        PartCycleBaseline(
            material_no="PART_B002", material_desc="机身焊接件",
            core_part_name="焊接件", machine_model="MC1-80",
            ref_batch_qty=Decimal("1"), cycle_days=Decimal("8"),
            unit_cycle_days=Decimal("8"), is_active=True,
        ),
    ...
    ok_parts = (
        await db_session.execute(
            select(PartScheduleResult).where(PartScheduleResult.order_line_id == order_ok.id)
        )
    ).scalars().all()
    fail_parts = (
        await db_session.execute(
            select(PartScheduleResult).where(PartScheduleResult.order_line_id == order_fail.id)
        )
    ).scalars().all()
...
async def test_schedule_order_precheck_counts_recursive_self_made_parts(db_session):
    order = SalesPlanOrderLineSrc(
        sap_code="SAPREC", sap_line_no="10",
        contract_no="HTREC", customer_name="客户REC",
        product_model="MC1-80", product_series="MC1",
        material_no="MACHREC", quantity=Decimal("1"),
        order_no="SOREC",
        confirmed_delivery_date=datetime(2026, 6, 30),
        drawing_released=True,
    )
    ...
    db_session.add_all([
        BomRelationSrc(
            machine_material_no="MACHREC", plant="1000",
            material_no="MACHREC", bom_component_no="ASM_REC",
            bom_component_desc="机身MC1-80", bom_level=1,
            is_top_level=True, is_self_made=True, part_type="自制件",
        ),
        BomRelationSrc(
            machine_material_no="MACHREC", plant="1000",
            material_no="ASM_REC", bom_component_no="SUB_NON_SELF",
            bom_component_desc="外购子总成", bom_level=2,
            is_self_made=False, part_type="外购件",
        ),
        BomRelationSrc(
            machine_material_no="MACHREC", plant="1000",
            material_no="SUB_NON_SELF", bom_component_no="PART_REC",
            bom_component_desc="递归关键件", bom_level=3,
    ...
    db_session.add_all([
        ...
        PartCycleBaseline(
            material_no="PART_REC", material_desc="递归关键件",
            core_part_name="递归关键件", machine_model="MC1-80",
            ref_batch_qty=Decimal("1"), cycle_days=Decimal("9"),
            unit_cycle_days=Decimal("9"), is_active=True,
        ),
    ])
    await db_session.commit()
