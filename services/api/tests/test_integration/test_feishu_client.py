...
async def test_search_records(client):
    ...
    respx.post("https://open.feishu.cn/open-apis/bitable/v1/apps/app123/tables/tbl456/records/search").mock(
        return_value=httpx.Response(
            200,
            json={
                "code": 0,
    ...
    items, has_more, _page_token, total = await client.search_records(
        app_token="app123",
        table_id="tbl456",
...
async def test_search_with_filter(client):
    ...
    respx.post("https://open.feishu.cn/open-apis/bitable/v1/apps/app123/tables/tbl456/records/search").mock(
        return_value=httpx.Response(
            200, json={"code": 0, "data": {"items": [], "has_more": False, "page_token": "", "total": 0}}
        )
    )
    _items, _has_more, _, total = await client.search_records(
        app_token="app123",
        table_id="tbl456",
        field_names=["生产订单号"],
        filter_config={
