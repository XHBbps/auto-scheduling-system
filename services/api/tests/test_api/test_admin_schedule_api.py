...
async def test_run_schedule_without_order_ids_auto_finds_schedulable_orders(app_client, db_session):
    order = SalesPlanOrderLineSrc(
        sap_code="SAP001-AUTO",
        sap_line_no="10",
        product_model="MC1-80",
        product_series="MC1",
        material_no="MACH001-AUTO",
        quantity=Decimal("1"),
        confirmed_delivery_date=datetime(2026, 3, 20),
        drawing_released=True,
        drawing_release_date=datetime(2026, 3, 1),
    ...
    db_session.add(
        BomRelationSrc(
            machine_material_no="MACH001-AUTO",
            plant="1000",
            material_no="MACH001-AUTO",
            bom_component_no="ASM001-AUTO",
...
async def test_run_schedule_missing_machine_cycle_marks_abnormal_and_creates_issue(app_client, db_session):
    ...
    db_session.add(
        BomRelationSrc(
            machine_material_no="MACH777",
            plant="1000",
            material_no="MACH777",
            bom_component_no="ASM777",
...
async def test_run_schedule_missing_part_cycle_marks_part_and_machine_abnormal(app_client, db_session):
    ...
    db_session.add(
        BomRelationSrc(
            machine_material_no="MACH005",
            plant="1000",
            material_no="MACH005",
            bom_component_no="ASM005",
            bom_component_desc="机身MC1-80",
            bom_level=1,
            is_self_made=True,
            part_type="自产件",
    ...
    part_schedule = (
        (await db_session.execute(select(PartScheduleResult).where(PartScheduleResult.order_line_id == order.id)))
        .scalars()
        .first()
    )
...
async def test_run_one_part_schedule_succeeds_for_single_order(app_client, db_session):
    order = SalesPlanOrderLineSrc(
        sap_code="SAP-PART-001",
        sap_line_no="10",
        product_model="MC1-80",
        product_series="MC1",
        material_no="MACH-PART-001",
        quantity=Decimal("1"),
        confirmed_delivery_date=datetime(2026, 3, 20),
        drawing_released=True,
        drawing_release_date=datetime(2026, 3, 1),
    ...
    db_session.add_all(
        [
            BomRelationSrc(
                machine_material_no="MACH-PART-001",
                plant="1000",
                material_no="MACH-PART-001",
                bom_component_no="ASM-PART-001",
                bom_component_desc="泵组总成",
                bom_level=1,
                is_self_made=True,
                part_type="整机",
            ),
            BomRelationSrc(
                machine_material_no="MACH-PART-001",
                plant="1000",
                material_no="ASM-PART-001",
                bom_component_no="PART-PART-001",
                bom_component_desc="泵体铸件",
                bom_level=2,
                is_self_made=True,
                part_type="零件",
            ),
    ...
    resp = await app_client.post(
        "/api/admin/schedule/run-one-part",
        json={
...
async def test_run_one_part_schedule_blocks_when_bom_missing(app_client, db_session):
    order = SalesPlanOrderLineSrc(
        sap_code="SAP-PART-404",
        sap_line_no="10",
        product_model="MC1-80",
        product_series="MC1",
        material_no="MACH-PART-404",
        quantity=Decimal("1"),
        confirmed_delivery_date=datetime(2026, 6, 30),
        drawing_released=True,
        drawing_release_date=datetime(2026, 3, 1),
...
async def test_run_one_part_schedule_blocks_when_part_data_missing(app_client, db_session):
    order = SalesPlanOrderLineSrc(
        sap_code="SAP-PART-NOCHILD",
        sap_line_no="10",
        product_model="MC1-80",
        product_series="MC1",
        material_no="MACH-PART-NOCHILD",
        quantity=Decimal("1"),
        confirmed_delivery_date=datetime(2026, 3, 20),
        drawing_released=True,
        drawing_release_date=datetime(2026, 3, 1),
    ...
    db_session.add(
        BomRelationSrc(
            machine_material_no="MACH-PART-NOCHILD",
            plant="1000",
            material_no="MACH-PART-NOCHILD",
            bom_component_no="ASM-PART-NOCHILD",
            bom_component_desc="泵组总成",
            bom_level=1,
            is_self_made=True,
            part_type="整机",
...
async def test_get_snapshot_observability_returns_healthy_summary(app_client, db_session):
    ...
    db_session.add(
        BomRelationSrc(
            machine_material_no="MACH-OBS-1",
            plant="1000",
            material_no="MACH-OBS-1",
            bom_component_no="ASM-OBS-1",
...
async def test_get_snapshot_observability_warns_when_snapshot_empty(app_client, db_session):
    ...
    order = SalesPlanOrderLineSrc(
        sap_code="SAP-OBS-EMPTY",
        sap_line_no="10",
        product_model="MC1-80",
        product_series="MC1",
        material_no="MACH-OBS-EMPTY",
        quantity=Decimal("1"),
        confirmed_delivery_date=datetime(2026, 3, 20),
        drawing_released=True,
        drawing_release_date=datetime(2026, 3, 1),
