  ...
  <div class="tech-card p-6 relative overflow-hidden group hover:border-brand/50 transition-colors duration-300">
    <div class="flex items-start justify-between gap-6 flex-wrap relative">
      <div>
        <div class="flex items-center gap-4 mb-3">
          <div class="w-12 h-12 rounded-2xl bg-surface-raised flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
          ...
          <div>
            ...
            <div class="text-text-muted text-sm mt-1">查看调度器启停状态、时区和下次计划执行时间。</div>
          </div>
        </div>
        <div class="flex items-center gap-3 flex-wrap mt-4">
          <AppStatusBadge v-bind="props.schedulerStateBadgeMeta" size="md" />
          <span class="text-xs text-text-muted bg-surface-page px-2 py-1 rounded-md border border-border">
      ...
      <div class="flex items-center gap-4 ml-auto bg-surface-page p-3 rounded-2xl border border-border">
        <div class="text-right mr-2">
          <div class="text-[10px] uppercase tracking-widest text-text-muted mb-1">AUTO SYNC</div>
          <div class="text-sm font-medium" :class="schedulerEnabled ? 'text-brand' : 'text-text-secondary'">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 mt-6 relative">
      <div
        ...
        :key="job.id"
        class="rounded-2xl border border-border bg-surface-page p-4 hover:border-brand/30 transition-colors"
      >
        <div class="flex items-center justify-between gap-3 mb-4">
          <div class="text-sm font-medium text-text-secondary">
            {{ props.schedulerJobNameMap[job.id] || job.name }}
          </div>
          <div class="flex items-center justify-center w-6 h-6 rounded-full bg-surface">
            <span
              class="w-2 h-2 rounded-full"
              :class="schedulerEnabled ? 'bg-brand shadow-[0_0_8px_rgba(130,214,149,0.8)]' : 'bg-text-muted'"
            />
          </div>
        </div>
        <div class="text-[10px] uppercase tracking-widest text-text-muted mb-1">NEXT RUN</div>
